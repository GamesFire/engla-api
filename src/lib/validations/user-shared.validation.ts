...
export const emailSchema = z
  ...
  .max(ValidationLimits.USER.EMAIL_MAX, { message: 'Email is too long' })
  .pipe(z.email({ message: 'Invalid email format' }));

export const nameSchema = z
  .string()
  .trim()
  .min(ValidationLimits.USER.NAME_MIN, { message: 'Name is too short' })
  .max(ValidationLimits.USER.NAME_MAX, { message: 'Name is too long' })
...
export const avatarSchema = z
  .string()
  .trim()
  .max(ValidationLimits.URL.MAX, { message: 'URL is too long' })
